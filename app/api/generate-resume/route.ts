import type { ResumeStyle, Provider } from '../../../types/resume';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!Array.isArray(body?.education))
      validationErrors.push("education is not an array");
    if (!Array.isArray(body?.workExperience))
      validationErrors.push("workExperience is not an array");
    ...
    const toProvider = (val: string | undefined): Provider =>
      val === 'huggingface' || val === 'groq' || val === 'auto' ? val : 'huggingface';
    const aiProvider: Provider = toProvider(process.env.AI_PROVIDER);
    let aiModel: string | undefined;
    if (aiProvider === 'huggingface' || aiProvider === 'auto') {
    ...
    const t4 = Date.now();
    console.debug(
      "[generate-resume] buildResumeHTML completed in",
      t4 - t3,
      "ms",
      "html length:",
      html.length
    ...
    const t6 = Date.now();
    console.debug(
      "[generate-resume] htmlToPDFBuffer completed in",
      t6 - t5,
      "ms"
    );

    const filename = `${safeFileName(
      polished.personalInfo.firstName + "-" + polished.personalInfo.lastName
    )}-resume.pdf`;
